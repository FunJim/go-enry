...
var ContentHeuristics = map[string]*Heuristics{
	...
	".action": &Heuristics{
		rule.And(
			rule.MatchingLanguages("ROS Interface"),
			rule.Or(
				rule.MatchingLanguages(""),
				regex.MustCompileMultiline(`^[a-z][a-z0-9_]*(?:\/[a-zA-Z]\w*)?(?:<=\d+)?(?:\[(?:<=\d+)?\])?\s+\w+\b`),
			),
			rule.Or(
				rule.MatchingLanguages(""),
			...
			rule.Not(
				rule.MatchingLanguages(""),
				regex.MustCompileMultiline(`^\s*[^\s@#\w-]`),
			),
		),
	},
	".al": &Heuristics{
	...
	".as": &Heuristics{
		rule.Or(
			...
			regex.MustCompileMultiline(`^\s*(?:package(?:\s+[\w.]+)?\s+(?:\{|$)|import\s+[\w.*]+\s*;|(?:intrinsic\s+)class\s+[\w<>.]+|\s+class\s+extends\s+[\w<>.]+|(?:(?:public|protected|private|static)\s+)*(?:(?:var|const|local)\s+\w+\s*:\s*[\w<>.]+(?:\s*=.*)?\s*;|function\s+\w+\s*\((?:\s*\w+\s*:\s*[\w<>.]+\s*(,\s*\w+\s*:\s*[\w<>.]+\s*)*)?\)))`),
		),
	},
	".asc": &Heuristics{
		rule.Or(
	...
	".asm": &Heuristics{
		rule.Or(
			rule.MatchingLanguages("Assembly"),
			regex.MustCompileMultiline(`(?i)mov\s+[^\s]{2,},`),
		),
		rule.Or(
	...
	".bas": &Heuristics{
		rule.Or(
			rule.MatchingLanguages("B4X"),
			regex.MustCompileMultiline(`\A\W{0,3}(?:.*(?:\r?\n|\r)){0,9}B4(?:J|A|R|i)=true`),
		),
		rule.Or(
			rule.MatchingLanguages("FreeBASIC"),
			regex.MustCompileMultiline(`(?i)^[ \t]*#(?:define|endif|endmacro|ifn?def|include|lang|macro|pragma)(?:$|\s)|(?i)^[ \t]*dim( shared)? [a-z_][a-z0-9_]* as [a-z_][a-z0-9_]* ptr`),
		),
		rule.And(
			rule.MatchingLanguages("FreeBASIC"),
			rule.Or(
				rule.MatchingLanguages(""),
				regex.MustCompileMultiline(`(?i)^[ \t]*return `),
			),
			rule.Not(
				rule.MatchingLanguages(""),
				regex.MustCompileMultiline(`(?i)[ \t]*gosub `),
			),
		),
		rule.Or(
	...
	".bs": &Heuristics{
		rule.Or(
			...
			regex.MustCompileMultiline(`^(?i:<pre\s+class)\s*=\s*('|\"|\b)metadata\b('|\"|\b)[^>\r\n]*>`),
		),
		rule.Or(
			rule.MatchingLanguages("BrighterScript"),
			regex.MustCompileMultiline(`(?i:^\s*(?:sub\s*\w+\(.*?\))|(?::\s*sub\(.*?\))$)|(?i:^\s*(end\ssub)$)|(?i:^\s*(?:function\s*\w+\(.*?\)\s*as\s*\w*)|(?::\s*function\(.*?\)\s*as\s*\w*)$)|(?i:^\s*(end\sfunction)$)`),
		),
		rule.Or(
	...
	".bst": &Heuristics{
		rule.Or(
			rule.MatchingLanguages("BibTeX Style"),
			regex.MustCompileMultiline(`ENTRY\s*\{`),
		),
		rule.Always(
			rule.MatchingLanguages("BuildStream"),
		),
	},
	".builds": &Heuristics{
		rule.Or(
	...
	".cairo": &Heuristics{
		rule.Or(
			rule.MatchingLanguages("Cairo Zero"),
			regex.MustCompileMultiline(`(^(\s*)%lang(\s+)([A-Za-z0-9_]+))|(^(\s*)%builtins(\s+)([A-Za-z0-9_]+\s*)*$)|(^(\s*)from(\s+)starkware\.(cairo|starknet)\.([A-Za-z0-9_.\s]+?)import)|(,\s*ap\+\+;$)|(;\s*ap\+\+$)`),
		),
		rule.Always(
			rule.MatchingLanguages("Cairo"),
		),
	},
	".ch": &Heuristics{
		rule.Or(
	...
	".csc": &Heuristics{
		rule.Or(
			...
			regex.MustCompileMultiline(`^\s*#\s*(?:using|insert|include|define|namespace)[ \t]+\w|^\s*((?:autoexec|private)\s+){0,2}function\s+((?:autoexec|private)\s+){0,2}\w+\s*\(|\b(?:level|self)[ \t]+thread[ \t]+(?:\[\[[ \t]*(\w+\.)+[ \t]*\]\]|\w+)[ \t]*\([^\r\n\)]*\)[ \t]*;|^[ \t]*#[ \t]*(?:precache|using_animtree)[ \t]*\(`),
		),
	},
	".csl": &Heuristics{
		rule.Or(
	...
	".es": &Heuristics{
		...
		rule.Or(
			...
			regex.MustCompileMultiline(`\/\/|["']use strict["']|export\s+default\s|\/\*(?:.|[\r\n])*?\*\/`),
		),
	},
	".ex": &Heuristics{
		rule.Or(
	...
	".frm": &Heuristics{
		...
		rule.Or(
			...
			regex.MustCompileMultiline(`\ATYPE=VIEW`),
		),
	},
	".fs": &Heuristics{
		rule.Or(
	...
	".ftl": &Heuristics{
		rule.Or(
			...
			regex.MustCompileMultiline(`^(?:<|[a-zA-Z-][a-zA-Z0-9_-]+[ \t]+\w)|\$\{\w+[^\r\n]*?\}|^[ \t]*(?:<#--.*?-->|<#(?:[a-z]+)\s[^>]*>.*?</#(?:[a-z]+)>|\[#--.*?--\]|\[#(?:[a-z]+)\s[^\]]*\].*?\[#(?:[a-z]+)\])`),
		),
		rule.Or(
	...
	".i": &Heuristics{
		...
		rule.Or(
			rule.MatchingLanguages("Assembly"),
			regex.MustCompileMultiline(`(?i)mov\s+[^\s]+,|^\s+(i?)db\s+[a-z\d]`),
		),
	},
	".ice": &Heuristics{
		rule.Or(
	...
	".inc": &Heuristics{
		...
		rule.Or(
			...
			regex.MustCompileMultiline(`^\s*include\s*\(\s*(?:"|')[\\/\w\-\.:\s]+\.(?:nasl|inc)\s*(?:"|')\s*\)\s*;|^\s*(?:global|local)_var\s+(?:\w+(?:\s*=\s*[\w\-"']+)?\s*)(?:,\s*\w+(?:\s*=\s*[\w\-"']+)?\s*)*\s*;|^\s*namespace\s+\w+\s*\{|^\s*object\s+\w+\s*(?:extends\s+\w+(?:::\w+)?)?\s*\{|^\s*(?:public\s+|private\s+|\s*)function\s+\w+\s*\([\w\s,]*\)\s*\{`),
		),
		rule.Or(
		...
		rule.Or(
			rule.MatchingLanguages("Assembly"),
			regex.MustCompileMultiline(`^(?i)[^"m]*mov\s+[^\s]+,|^\s+(?i)db\s+[a-z\d]`),
		),
	},
	".json": &Heuristics{
		rule.Or(
	...
	".lp": &Heuristics{
		rule.Or(
			rule.MatchingLanguages("Linear Programming"),
			regex.MustCompileMultiline(`^(?i:minimize|minimum|min|maximize|maximum|max)(?i:\s+multi-objectives)?$`),
		),
		rule.Always(
			rule.MatchingLanguages("Answer Set Programming"),
		),
	},
	".ls": &Heuristics{
		rule.Or(
	...
	".mc": &Heuristics{
		rule.Or(
			...
			regex.MustCompileMultiline(`(?i)^[ \t]*(\/\*\s*)?MessageId=|^\.$`),
		),
		rule.Or(
			rule.MatchingLanguages("M4"),
	...
	".msg": &Heuristics{
		rule.Or(
			rule.MatchingLanguages("OMNeT++ MSG"),
			regex.MustCompileMultiline(`^cplusplus\(?[\S]*\)?[\s]*\{?\{?|^namespace[\s]+([^.\s]*\.)*[^.\s]*;|^struct[\s]+[\S]+|^message[\s]+[\S]+(extends )?[\S]*[\s]*|^packet[\s]+[\S]+|^class[\s]+[\S]+(extends )?[\S]*[\s]*|^enum[\s]+[\S]+|^import ([^.\s]*\.)*[^.\s]*;`),
		),
		rule.And(
			rule.MatchingLanguages("ROS Interface"),
			rule.Or(
	...
	".ncl": &Heuristics{
		...
		rule.Or(
			rule.MatchingLanguages("Nickel"),
			regex.MustCompileMultiline(`^let(?:\srec)?(?:\s[a-zA-Z_][a-zA-Z0-9_]*)?|^import\s"[^"]+"\s+as\s|std\.[a-zA-Z_][a-zA-Z0-9_]*\.`),
		),
		rule.Or(
			rule.MatchingLanguages("Text"),
		...
		rule.Or(
			rule.MatchingLanguages("NCL"),
			regex.MustCompileMultiline(`^load "|^begin$|[0-9]\.$|^;`),
		),
	},
	".nl": &Heuristics{
		rule.Or(
	...
	".plist": &Heuristics{
		rule.Or(
			...
			regex.MustCompileMultiline(`^\s*(?:<\?xml\s|<!DOCTYPE\s+plist|<plist(?:\s+version\s*=\s*["']\d+(?:\.\d+)?["'])?\s*>\s*$)`),
		),
		rule.Always(
	...
	".resource": &Heuristics{
		rule.Or(
			rule.MatchingLanguages("RobotFramework"),
			regex.MustCompileMultiline(`^\*{3} (Settings|Variables|Keywords) \*{3}$`),
		),
	},
	".rno": &Heuristics{
		rule.Or(
			rule.MatchingLanguages("RUNOFF"),
			regex.MustCompileMultiline(`(?i:^\.!|^\f|\f$|^\.end lit(?:eral)?\b|^\.[a-zA-Z].*?;\.[a-zA-Z](?:[; \t])|\^\*[^\s*][^*]*\\\*(?:$|\s)|^\.c;[ \t]*\w+)`),
		),
		rule.Or(
			rule.MatchingLanguages("Roff"),
			regex.MustCompileMultiline(`^\.\\" `),
	...
	".s": &Heuristics{
		rule.Or(
			rule.MatchingLanguages("Unix Assembly"),
			regex.MustCompileMultiline(`(?i:mov[lq]?)\s+[%\$]`),
		),
		rule.Or(
			rule.MatchingLanguages("Assembly"),
			regex.MustCompileMultiline(`(?i:mov)\s+[^\s%]{2,},`),
	...
	".scm": &Heuristics{
		rule.Or(
			rule.MatchingLanguages("Tree-sitter Query"),
			regex.MustCompileMultiline(`\(#[\w-]+[!\?]|(?:[\)\]]\s*[\*\+\?](?:\s|$))|(?:^\s*\w+:\s*[\(\[\"])|\(#(?:set!|(?:not-)?(?:any-of|match)\?)|@[\w.-]+(?:\)\s|$)`),
		),
		rule.Or(
			rule.MatchingLanguages("Scheme"),
			regex.MustCompileMultiline(`(?:'[\(\*#]|\w->\w|\.\.\.[\s\)]|\([+\-:<>\/=~\)]|~>|[#`+"`"+`]\(|#:\w)|^\s*\((?:define\*?|import|library|lambda)`),
		),
	},
	".sol": &Heuristics{
		rule.Or(
			rule.MatchingLanguages("Solidity"),
			regex.MustCompileMultiline(`\bpragma\s+solidity\b|\b(?:abstract\s+)?contract\s+[a-zA-Z$_][a-zA-Z0-9$_]*(?:\s+is\s+(?:[a-zA-Z0-9$_][^\{]*?)?)?\s*\{`),
		),
		rule.Or(
			rule.MatchingLanguages("Gerber Image"),
			regex.MustCompileMultiline(`^[DGMT][0-9]{2}\*(?:\r?\n|\r)`),
	...
	".st": &Heuristics{
		rule.Or(
			...
			regex.MustCompileMultiline(`\$\w+[($]|.!\s*.+?\s*!.|<!\s*.+?\s*!>|\[!\s*.+?\s*!\]|\{!\s*.+?\s*!\}`),
		),
		rule.Or(
			rule.MatchingLanguages("Smalltalk"),
	...
	".svx": &Heuristics{
		rule.Or(
			rule.MatchingLanguages("Survex data"),
			regex.MustCompileMultiline(`\A(;|\*[^*]+$)`),
		),
		rule.Always(
			rule.MatchingLanguages("mdsvex"),
		),
	},
	".sw": &Heuristics{
		rule.Or(
	...
	".tsp": &Heuristics{
		rule.Or(
			rule.MatchingLanguages("TypeSpec"),
			regex.MustCompileMultiline(`^(import|using|namespace|interface|op|model|scalar|alias|union|enum)\s`),
		),
		rule.Or(
			rule.MatchingLanguages("TSPLIB data"),
			regex.MustCompileMultiline(`^(NAME|TYPE|DIMENSION|EDGE_WEIGHT_TYPE|EDGE_WEIGHT_FORMAT)\s*:`),
		),
	},
	".tst": &Heuristics{
		rule.Or(
	...
	".txt": &Heuristics{
		...
		rule.Or(
			rule.MatchingLanguages("Hosts File"),
			regex.MustCompileRuby(`(?xi) ^
...
\]`),
	...
	".url": &Heuristics{
		rule.Or(
			rule.MatchingLanguages("INI"),
			regex.MustCompileMultiline(`^\[InternetShortcut\](?:\r?\n|\r)([^\s\[][^\r\n]*(?:\r?\n|\r)){0,20}URL=`),
		),
	},
	".v": &Heuristics{
		rule.Or(
			rule.MatchingLanguages("Rocq Prover"),
	...
	".vcf": &Heuristics{
		rule.Or(
			rule.MatchingLanguages("TSV"),
			regex.MustCompileMultiline(`\A##fileformat=VCF`),
		),
		rule.Or(
			rule.MatchingLanguages("vCard"),
			regex.MustCompileMultiline(`\ABEGIN:VCARD`),
		),
	},
	".w": &Heuristics{
		rule.Or(
